...
void Camera::loadFPNCorrection(FrameGeometry *geometry, const UInt16 *fpnBuffer, UInt32 framesToAverage)
{
	...
	UInt32 columnScale = (geometry->vRes * framesToAverage);
	UInt32 maxColumn = 0;

	/* Generate the column averages. */
	...
	for (int col = 0; col < geometry->hRes; col++) {
		if ((fpnColumns[col] / columnScale) > maxColumn) maxColumn = (fpnColumns[col] / columnScale);
	}

	/* Load using 3-point polynomial calibration */
	if (gpmc->read16(DISPLAY_GAIN_CONTROL_ADDR) & DISPLAY_GAIN_CONTROL_3POINT) {
		/*
		...
		for (int col = 0; col < geometry->hRes; col++) {
			...
			Int64 fpnCurved = -(square * curve) / (columnScale << (COL_CURVE_FRAC_BITS - COL_GAIN_FRAC_BITS));
			Int16 offset = (fpnLinear + fpnCurved) / (columnScale << COL_GAIN_FRAC_BITS);
	...
	/* Load using 2-point calibration */
	else {
		/* The AC component of each column remains as the per-pixel FPN. */
...
void Camera::computeGainColumns(FrameGeometry *geometry, UInt32 wordAddress, const struct timespec *interval, const char *gName)
{
	...
	if (gName) {
		double gainCorrection[LUX1310_HRES_INCREMENT];
		double curveCorrection[LUX1310_HRES_INCREMENT];
		QString filename;
		QFile fp;
		...
		qDebug() << "Saving column gain to" << filename;
		fp.setFileName(filename);
		fp.open(QIODevice::WriteOnly);
		if(fp.isOpen()) {
		...
		qDebug() << "Saving column curvature to" << filename;
		fp.setFileName(filename);
		fp.open(QIODevice::WriteOnly);
		if(fp.isOpen()) {
	...
	/* Load gain calibration results. */
	gpmc->write16(DISPLAY_GAIN_CONTROL_ADDR, (USE_3POINT_GAIN ? DISPLAY_GAIN_CONTROL_3POINT : 0));
	for (int col = 0; col < geometry->hRes; col++) {
...
void Camera::loadColGainFromFile(void)
{
	...
	if (colCurveFile.exists() && colCurveFile.isFile()) {
		...
		/* Load and enable 3-point calibration */
